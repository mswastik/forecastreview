...
def cont(df,l2fc,l0fc,cc,cp):  #POLARS
    if not df.empty:
        ...
        acc=df.copy()
        acc=acc[acc["CatalogNumber"].isin(cc)]
        #print(acc.columns)
        ...
        #acc=acc[(acc['SALES_DATE']>=pd.Timestamp.today()-pd.offsets.MonthBegin(24,normalize=True)) & (acc['SALES_DATE']<=pd.Timestamp.today()+pd.offsets.MonthBegin(24,normalize=True))]
        acc1=acc[(acc['SALES_DATE']>=pd.Timestamp.today()-pd.offsets.MonthBegin(4,normalize=True)) & (acc['SALES_DATE']<pd.Timestamp.today()-pd.offsets.MonthBegin(1,normalize=True))]
        acc1['orders cont']=acc1['`Act Orders Rev']/acc1['`Act Orders Rev'].sum()*100
        acc1['var cont']=acc1['L2 Abs Var']/acc1['L2 Abs Var'].sum()*100
        acc=acc[['IBP Level 5','CatalogNumber','SALES_DATE',l0fc,l2fc,'`Act Orders Rev']]
        acc1=acc1[['CatalogNumber','SALES_DATE','`Act Orders Rev',l0fc,l2fc,'orders cont','var cont']]
        #acc=acc.merge(acc1,on=['CatalogNumber','SALES_DATE'],how='left')
        acc=acc.melt(['IBP Level 5','CatalogNumber','SALES_DATE'])
        print(acc['SALES_DATE'].unique())
        acc.rename(columns={'variable':'type'},inplace=True)
        #acc.sort_values('`Act Orders Rev',ascending=False)
        its=alt.selection_point(fields=['CatalogNumber'],name='brush')
        c1=alt.Chart(acc1[acc1['SALES_DATE']==datetime(today.year,today.month,1)-relativedelta(months=2)]).mark_circle(size=85).encode(
        ...
        c2=alt.Chart(acc).mark_line(point=True).encode(alt.X('SALES_DATE:T', axis=alt.Axis(format="%b-%y")),y='sum(value):Q',color='type',tooltip=['sum(value):Q','SALES_DATE']).properties(height=390,width=600).transform_filter(its)
